...
$sql = "SELECT id, name, email, phone, preferred_date, preferred_time, message FROM consultations ORDER BY preferred_date DESC";
...
<head>
    ...
    <style>
        ...
        th {
            background-color: rgb(186, 61, 117);
            color: white;
        }
...
if ($result->num_rows > 0) {
    echo "<table id='consultationTable'>";
    echo "<tr>
            <th>ID</th>
            <th>Name</th>
            <th>Email</th>
    ...
    echo "<p style='text-align:center;'>No consultations found.</p>";
}
$conn->close();
?>
...
async function downloadPDF() {
    ...
    doc.text('Consultation Report', 14, 20);
    
    const table = [];
    document.querySelectorAll("#consultationTable tr").forEach(row => {
        const rowData = [];
...
function downloadExcel() {
    ...
    downloadLink.download = "consultation_report.csv";
    downloadLink.click();
}

function downloadWord() {
    const header = "<html><head><meta charset='utf-8'><title>Consultation Report</title></head><body>";
    const footer = "</body></html>";
    const table = document.getElementById("consultationTable").outerHTML;
    const sourceHTML = header + table + footer;

    const blob = new Blob(['\ufeff', sourceHTML], { type: 'application/msword' });
    const downloadLink = document.createElement("a");
    downloadLink.href = URL.createObjectURL(blob);
    downloadLink.download = "consultation_report.doc";
    downloadLink.click();
}

function printTable() {
    const tableHTML = document.getElementById("consultationTable").outerHTML;
        ...
        <html>
            <head>
                ...
                <style>
                    ...
                    th { background-color: #ba3d75; color: white; }
